...
import { getFirestore, collection, onSnapshot, getDocs, query, orderBy, limit } from "https://www.gstatic.com/firebasejs/12.1.0/firebase-firestore.js";
...
// Init Firebase & Global variables
let homeMap, scheduleMap;
let originMarker, destMarker, routeLine;
let currentRouteIndex = 0;
let routeInterval;

// === USER AUTHENTICATION & PAGE NAVIGATION ===
...
function showPage(pageId) {
    ...
    if (target) {
        ...
        if (pageId === 'notifications') setTimeout(initNotificationsPage, 50);
    } else {
...
function updateClock() {
    ...
    if (currentDateTimeEl) {
        currentDateTimeEl.textContent = `${currentDate} ${currentTime}`;
    }
    const currentDateTimeCompactEl = document.getElementById('currentDateTimeCompact');
    if (currentDateTimeCompactEl) {
        currentDateTimeCompactEl.textContent = `${currentDate}, ${compactTime}`;
    }
}
...
const routes = {
    ...
    "dewildt-pretoria": { name: "De Wildt → Pretoria", origin: "De Wildt", destination: "Pretoria", substations: [{name: "De-Wildt", stopTime: 2 }, { name: "Taillardshoop", travelTime: 5, stopTime: 2 }, { name: "Ga-rankuwa", travelTime: 5, stopTime: 2 }, { name: "Rosslyn", travelTime: 5, stopTime: 2 }, { name: "Wintersnest", travelTime: 5, stopTime: 2 }, { name: "Wolmerton", travelTime: 5, stopTime: 2 }, { name: "Pretoria-N", travelTime: 5, stopTime: 2 }, { name: "Mountain view", travelTime: 5, stopTime: 2 }, { name: "Hercules", travelTime: 5, stopTime: 2 }, { name: "Pretoria West", travelTime: 5, stopTime: 2 }, { name: "Pretoria", travelTime: 5 }], price: "R6.80", originCoords: [-25.61248, 27.91062], destCoords: [-25.7548, 28.1868], color: '#3498db' },
    "pretoria-dewildt": { name: "Pretoria → De Wildt", origin: "Pretoria", destination: "De Wildt", substations: [{name: "Pretoria", stopTime: 2 }, { name: "Pretria-West", travelTime: 5, stopTime: 2 }, { name: "Hercules", travelTime: 5, stopTime: 2 }, { name: "Mountain-View", travelTime: 5, stopTime: 2 }, { name: "Pretoria-N", travelTime: 5, stopTime: 2 }, { name: "Wolmerton", travelTime: 5, stopTime: 2 }, { name: "Wintersnest", travelTime: 5, stopTime: 2 }, { name: "Rosslyn", travelTime: 5, stopTime: 2 }, {name: "Taillardrshoop", travelTime: 5, stopTime: 2}, { name: "Ga-Rankuwa", travelTime: 5 }, { name: "De-Wildt", travelTime: 5 }], price: "R7.20", originCoords: [-25.7548, 28.1868], destCoords: [-25.61248, 27.91062], color: '#2ecc71' }
};
...
function setupFAQEvents() {
    ...
    searchInput.addEventListener('keyup', (e) => {
        if (e.key === 'Enter') renderFAQs(document.querySelector('.category-btn.active').dataset.category, searchInput.value.toLowerCase());
    });
}
...
function updateSystemStatus() {
    const criticalNotifications = notifications.filter(notif => notif.urgency === 'critical' && new Date(notif.timestamp.toDate()) > new Date(Date.now() - 24 * 60 * 60 * 1000));
    if (criticalNotifications.length > 0) {
        systemStatus.textContent = 'Service Disrupted';
        systemStatus.className = 'status-badge status-disrupted';
    } else if (notifications.some(notif => notif.urgency === 'high' && new Date(notif.timestamp.toDate()) > new Date(Date.now() - 24 * 60 * 60 * 1000))) {
...
function renderNotifications() {
    ...
    if (filteredNotifications.length === 0) {
        notificationsList.innerHTML = `<div class="no-notifications"><h3>No notifications found</h3><p>There are currently no ${currentFilter !== 'all' ? currentFilter : ''} notifications.</p></div>`;
        return;
    }

    notificationsList.innerHTML = filteredNotifications.map(notif => {
        const urgencyClass = notif.urgency === 'critical' ? 'critical' : notif.urgency === 'high' ? 'high' : '';
        const lineBadges = notif.lines ? notif.lines.map(line => `<span class="line-badge line-${line}">${line.charAt(0).toUpperCase() + line.slice(1)} Line</span>`).join(' ') : '';
        return `<div class="notification-card ${urgencyClass}"><div class="notification-header"><div class="notification-title">${notif.title}</div><div class="notification-time">${formatDate(notif.timestamp)}</div></div><div class="notification-content"><p>${notif.message}</p></div><div class="notification-meta">${lineBadges ? `<div>Affected lines: ${lineBadges}</div>` : ''}<div>Priority: <span class="status-badge ${urgencyClass}">${notif.urgency}</span></div></div></div>`;
    }).join('');
}
...
function fetchNotifications() {
    ...
    getDocs(q)
        ...
        .catch((error) => {
            ...
            notificationsList.innerHTML = `<div class="no-notifications"><h3>Error loading notifications</h3><p>Please try again later.</p></div>`;
        });
}
...
function setupRealTimeNotifications() {
    ...
    const qRealTime = query(notificationsCollectionRef, orderBy("timestamp", "desc"), limit(10));
